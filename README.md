...
- **Bash path blocking intentionally fails closed on path-shaped tokens.** A `PreToolUse` hook receives the raw shell command string, not the program and operands the shell will ultimately resolve. Agent Guard therefore checks every command token against `deny-read-paths.txt`; a benign word such as `foo.key` / `foo.pem`, or a jq selector such as `.key`, can be blocked even when no file exists and nothing would be read ([#99](https://github.com/JeongJaeSoon/agent-guard/issues/99)). The gate does not exempt operands based on an apparent command name: aliases, functions, wrappers, `PATH`, pipes, and compound commands can make a token under `echo`, `printf`, or `jq` become a real read target at execution time. When the command is genuinely benign, use an equivalent expression that is not path-shaped (for example, `jq '.["key"]' data.json`); do not weaken the deny list merely to silence this false positive. Direct Read/Grep/Glob requests still use their explicit path fields and are unaffected by this shell-string ambiguity.

...
Because the plugin (auto-updated by `claude plugin update`) and the standalone CLI (what `agx` and the bang guard actually run) update independently, a plugin-only update can silently leave `!`-command masking on older rules. A Claude Code `SessionStart` hook compares the two version strings at session start and shows a **non-blocking warning** on mismatch; it stays silent when they match or when no CLI is installed.
